...
Минималистичный сайт c [моими](https://t.me/isayev_ivan) [«вечно-зелёными»](https://arseny.page/newsletter/letter-23/) заметками о заточке ножей в стиле [Энди Матушака](https://notes.andymatuschak.org/About_these_notes). **Сами заметки ещё не готовы, тут тестовые...**
...
* «горизонтальная лента» колонок: ссылки-заметки открываются в колонке справа рядом
...
# PWA работает, можно тестить офлайн. 
# Используй после npm run build, чтобы проверить «как на проде» 
# (минификация, PWA, правильные пути и т.п.).
# По сути — статический сервер с заголовками как у Vite.
npm run preview

# Запускает строгую проверку типов для .ts и .vue (через vue-tsc) без генерации файлов.
# Ловит ошибки типизации, несовместимые пропсы, неправильные эмиты и т.д.
# Это то, что сборщик Vite может «пропустить».
# Когда запускать: перед коммитом/PR и в CI. Если падает — сборка тоже должна считаться «красной».
npm run typecheck

# Прогоняет ESLint по всему проекту (включая SFC). 
# Ловит проблемные конструкции и несоответствия кодстайлу 
#  (импорты, неиспользуемые переменные, опасные паттерны).
# Выдаёт ошибки/предупреждения, но не меняет файлы.
npm run lint

# То же, что lint, но с авто-исправлениями (где возможно): 
# починит сортировку импортов, уберёт неиспользуемые, расставит пробелы по правилам и т.п.
# Полезно запускать после больших правок.
npm run lint:fix

# Применяет Prettier ко всем файлам, которые он умеет (включая .vue, .ts, .json, .md). 
# Это чисто про внешний вид кода — кавычки, запятые, переносы.
# Мы подключили eslint-config-prettier, поэтому правила ESLint, 
# конфликтующие с форматированием, выключены — источником правды по стилю является Prettier.
npm run format

# Быстрая суммарная проверка: типы + линт. 
# Удобно ставить в CI/PR как «gate»: если check зелёный — можно собирать. 
# Локально тоже удобно перед пушем.
npm run check
```
...
- Разработка:
    - npm run dev — обычный Vite dev-сервер.
    - Периодически: npm run lint:fix и/или автосейв Prettier в IDE.
- Перед PR:
    - npm run check (типы + линт),
    - npm run build,
    - npm run preview — быстро посмотреть прод-режим.
- В CI:
    - запускать npm run check и затем npm run build.
